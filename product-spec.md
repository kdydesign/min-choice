...
* 초기 버전은 구글 로그인과 익명 체험 흐름을 우선 지원해야 한다.
...
* 익명 사용자 흐름을 지원하고 로그인 후 계정 연동 가능해야 한다.
* 카카오 로그인은 후속 TODO로 남기고 현재 운영 경로에서는 비활성화한다.
...
2. Google 로그인 버튼 UI를 LoginPage에 추가해.
...
- 부분 진행: 0 / 8
- 미진행: 0 / 8
- 추가 완료: Vitest 기반 핵심 순수 로직 단위 테스트 10개
...
- [ ] Kakao 로그인 연동 재개 _(Kakao 동의 항목/이메일 스코프 정책 확인 후 재도입)_
