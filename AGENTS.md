...
- 개월 수와 생년월일 선택 UI는 공통 `MobileSelect`를 재사용한다.
- 생년월일 월/연도 변경으로 유효하지 않은 일이 되면 해당 월의 마지막 일자로 보정한다.
...
- 과한 모션을 넣지 않는다.
- 큰 터치 영역을 유지한다.
- 모바일 선택 팝업이 열려 있는 동안 배경 스크롤을 잠근다.
- iOS Chrome/Safari 키보드 open/close 보정에서 앱 전체 높이를 `visualViewport.height`로 직접 줄이지 않는다.
- 고정 헤더/하단 메뉴 화면은 body 외부 스크롤과 내부 스크롤 컨테이너를 분리해서 관리한다.
